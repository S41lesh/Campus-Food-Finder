...
export default function EditFood() {
  ...
  const receivedData = route.params?.data1 || {};
    const navigation = useNavigation();
    const fromTime = receivedData.From.toDate();
    const toTime = receivedData.To.toDate();
    const [iname, setiname] = useState(receivedData.id);
    const [Price1, setPrice] = useState(receivedData.Price);
    const [imageUri, setImageUri] = useState(receivedData.image);
  ...
  const handleToTimeChange = (event, selected) => {
    ...
    const uploadImage = async () => {
      try {
        ...
        await handleSubmit(downloadURL)
      } catch (error) {
  ...
  const handleSubmit = async (url) => {
    ...
    if (url) {
      const collectionRef = doc(categorycollection, receivedData.docu);
    ...
    <View className="flex-1 bg-white" style={{backgroundColor: themeColors.bg}}>
      ...
      >
        <View className="form space-y-2">
            <Text className="text-black-700 ml-4 mb-5">Food Name : {iname}</Text>
